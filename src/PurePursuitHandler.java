...
public class PurePursuitHandler {
    ...
    public static Point getLookaheadPoint(Point robotPosition){
        ...
        for (int i = 0; i < path.size() - 1; i++){
            if (distanceFormula(path.get(i), robotPosition) < lookaheadDistance)
                path.get(i).setPassed();
            ...
            if (discriminant < 0 || p1.equals(p2)) {
                /*if (i == path.size()-1) {
                    ...
                    if (discriminant < 0) {
                        lookahead = path.get(path.size() - 1);
                        //Robot is off the path and the next point is the last point in the track..
                    }
                }
                else*/ continue;
            }
